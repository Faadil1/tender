...
export function verifyEconomicIdentity(
  contribution: Contribution,
  acceptance: AcceptanceEvidence,
  policy: SettlementPolicy,
  override: EconomicOverride = {}
) {
  const canonicalClaimId = claimIdFor(contribution, acceptance, policy);
  const nextContribution: Contribution = {
    ...contribution,
    recipients: contribution.recipients.map((recipient) => ({ ...recipient }))
  };
  ...
  if (override.amount !== undefined) {
    ...
    nextContribution.recipients = nextContribution.recipients.map((recipient, index) =>
      index === 0 ? { ...recipient, amount: normalized } : recipient
    );
  }
  ...
  if (override.recipient !== undefined) {
    ...
    nextContribution.recipients = nextContribution.recipients.map((recipient, index) =>
      index === 0 ? { ...recipient, wallet: override.recipient!.trim() } : recipient
    );
  }
